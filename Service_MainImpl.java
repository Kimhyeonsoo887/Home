...
@Service
public class Service_MainImpl implements Service_Main{
	@Autowired
	DAO_Main daoM;
	...
	public void signup(HttpServletRequest req, Model model) {
		...
		vo.setMember_phone(member_hp);
		vo.setMember_pw(encryptPw);
		
		
		int signupCnt = 0;
		
		signupCnt = daoM.signUp(vo);
	...
	//회원정보 수정 화면 
		...
		public void modifyView(HttpServletRequest req, Model model) {
			...
			//String pwd=daoM.pwdCheck(strId);
			
			//int selectCnt=0;
			//if(passwordEncoder.matches(strPwd, pwd)) {
				//selectCnt=1;
				VO_Member vo=daoM.readMember(strId);
		...
		public void updateMemberPro(HttpServletRequest req, Model model) {
			...
			vo.setMember_id(strId);
			vo.setMember_pw(encodepwd);
			vo.setMember_phone(member_hp);
			
			int updateCnt=daoM.updateMember(vo);
		...
		public void deleteMemberPro(HttpServletRequest req, Model model) {
			...
			if(passwordEncoder.matches(strPwd, pwd)) {
				selectCnt = 1;
				deleteCnt=daoM.deleteMember(strId);
			}
		...
		public void find_account(HttpServletRequest req, Model model) {
			...
			//이름의 유무를 확인하는 쿼리문
			findName = daoM.checkName(inputName);
			...
			if (findName != 0) {
				...
				info.put("member_name", inputName);
				info.put("member_hp", inputPhone);
				
				//이름, 번호가 일치한 정보가 존재하는지 확인하는 쿼리문
				findPhone = daoM.checkPhone(info);
				System.out.println("findPhone:" + findPhone);
				
				//만약 일치하는 정보가 존재한다면,
				if (findPhone != 0) {
					//아이디 값을 받아와 JSP에 보여준다.
					String member_id = daoM.findAccount(info);
		...
		public void change_password(HttpServletRequest req, Model model) {
			...
			checkCnt = daoM.checkAccount(member_id);
			System.out.println("checkCnt:" + checkCnt);
			if (checkCnt == 1) {
				String email = daoM.findEmail(member_id);
				daoM.sendPwEmail(member_id, email);
				//"등록하신 이메일로 전송되었습니다."
		...
		public void change_passoword_page_pro(HttpServletRequest req, Model model) {
			...
			changeCnt = daoM.changePassword(info);
